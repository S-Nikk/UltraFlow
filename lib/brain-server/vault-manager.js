...
class VaultManager {
   ...
   * Factors: type, recency, access frequency
   */
  calculateRelevancy(memory, currentContext = {}) {
    ...
    // Recency score (stronger decay, 0 after 60 days)
    if (memory.updated) {
    ...
    // Access frequency score (higher = more frequently accessed)
    const accessCount = memory.accessCount || 0;
    const accessScore = Math.min(1, accessCount / 10); // 10+ accesses = max score
    score = (score + accessScore) / 2;
  ...
  recordAccess(name) {
    ...
    if (memory) {
      ...
      // Recalculate relevancy after access
      memory.relevancy = this.calculateRelevancy(memory);
    }
  }


  /**
   * Move memory to vault by name and threshold
   */
  moveToVault(name, threshold = 0.3) {
    const index = this.vault.conscious.memories.findIndex(m => m.name === name);
    if (index === -1) {
      return { status: 'not_found', name };
    }

    const memory = this.vault.conscious.memories[index];
    const relevancy = this.calculateRelevancy(memory);

    if (relevancy < threshold) {
      // Move to vault
      this.vault.conscious.memories.splice(index, 1);
      this.vault.vault.entries.push({
        name: memory.name,
        type: memory.type,
        content: memory.content,
        relevancy,
...
// Export class for use by server.js
export { VaultManager };
export default VaultManager;

// CLI Interface (only runs when executed directly)
if (import.meta.url === `file://${process.argv[1]}` ||
    process.argv[1]?.endsWith('vault-manager.js')) {
