...
const command = new Command('register')
  ...
  .action(async (opts) => {
    ...
    if (opts.opencode || (!opts.claude)) {
      ...
      if (opencodeConfig) {
        try {
          ...
          console.error(chalk.red(`Failed to update OpenCode: ${e.message}`));
        }
      } else {
        // Create default
    ...
    if (opts.claude || (!opts.opencode)) {
      ...
      if (fs.existsSync(claudePath)) {
        try {
          ...
          console.error(chalk.red(`Failed to update Claude Code: ${e.message}`));
        }
      } else {
    ...
    if (registered.length > 0) {
      ...
      console.log('\nRun ' + chalk.cyan('npx ultraflow start') + ' to start services\n');
    } else {
