...
(async () => {
  const availablePort = await findAvailablePort(PORT_RANGE_START);
  if (!availablePort) {
    console.error('[Dashboard] No available ports in range');
    process.exit(1);
  }
  if (availablePort !== DEFAULT_PORT) {
    console.log(`[Dashboard] Port ${DEFAULT_PORT} busy, using port ${availablePort}`);
  }
  PORT = availablePort;
  server.listen(PORT, () => {
